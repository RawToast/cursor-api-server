...
async function handleRequest(request, response) {
  ...
  const sessionKey =
    typeof body.sessionKey === "string" && body.sessionKey ? body.sessionKey : crypto.randomUUID()
...
async function runLocalAgentBody(input, onRun, onEvent) {
  ...
  try {
    ...
    const prompt =
      agentEntry.cached && input.incrementalPrompt ? input.incrementalPrompt : input.prompt
...
function validateJsonSchemaValue(value, schema, path, rootSchema = schema, seenRefs = new Set()) {
  ...
  if (reference)
    return validateJsonSchemaValue(value, reference.schema, path, root, reference.seenRefs)
  ...
  if (
    ...
    !anyOf.some(
      (candidate) =>
        validateJsonSchemaValue(value, candidate, path, root, new Set(seenRefs)) === null,
    )
  ) {
  ...
  if (isRecord(schema.if) || typeof schema.if === "boolean") {
    const matchesIf =
      validateJsonSchemaValue(value, schema.if, path, root, new Set(seenRefs)) === null
  ...
  if (arrayLike) {
    ...
    if (isRecord(schema.contains) || typeof schema.contains === "boolean") {
      ...
      for (let index = 0; index < value.length; index += 1) {
        if (
          validateJsonSchemaValue(value[index], schema.contains, path, root, new Set(seenRefs)) ===
          null
        ) {
          matches += 1
...
function schemaEvaluatesObjectProperty(schema, key, rootSchema, value, seenRefs = new Set()) {
  ...
  if (isRecord(schema.properties) && Object.prototype.hasOwnProperty.call(schema.properties, key))
    return true
  ...
  if (isRecord(schema.dependentSchemas) && isRecord(value)) {
    for (const [dependency, dependentSchema] of Object.entries(schema.dependentSchemas)) {
      ...
      if (schemaEvaluatesObjectProperty(dependentSchema, key, rootSchema, value, new Set(seenRefs)))
        return true
    }
  }
  if (Array.isArray(schema.allOf)) {
    if (
      schema.allOf.some((candidate) =>
        schemaEvaluatesObjectProperty(candidate, key, rootSchema, value, new Set(seenRefs)),
      )
    )
      return true
  }
  for (const keyword of ["anyOf", "oneOf"]) {
    ...
    for (const candidate of schema[keyword]) {
      if (validateJsonSchemaValue(value, candidate, "$", rootSchema, new Set(seenRefs)) !== null)
        continue
      if (schemaEvaluatesObjectProperty(candidate, key, rootSchema, value, new Set(seenRefs)))
        return true
    }
  }
  if (isRecord(schema.if) || typeof schema.if === "boolean") {
    const matchesIf =
      validateJsonSchemaValue(value, schema.if, "$", rootSchema, new Set(seenRefs)) === null
    if (
      matchesIf &&
      schemaEvaluatesObjectProperty(schema.if, key, rootSchema, value, new Set(seenRefs))
    )
      return true
...
function mcpWrapperPayloadLooksComplete(args) {
  ...
  switch (toolName) {
    ...
    case "overwritefile":
      return (
        hasString(payload, "path", "filePath", "file_path", "targetFile", "target_file") &&
...
function isRetryableSDKRunError(error) {
  ...
  if (
    values.some(
      (value) =>
        value?.status === 429 || value?.status === 503 || value?.code === 8 || value?.code === 14,
    )
  )
    return true
  return values
