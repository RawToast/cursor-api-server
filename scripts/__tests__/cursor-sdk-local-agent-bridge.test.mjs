...
describe("Cursor SDK local-agent bridge", () => {
  ...
  it("treats opaque SDK error results as retryable but preserves explicit auth failures", () => {
    ...
    expect(
      sdkRunFailureSummary({
        status: "error",
        error: { message: "Missing or invalid authorization", code: "unauthorized" },
      }),
    ).toMatchObject({
  ...
  it("requires both provider and tool names for SDK MCP forwarding", () => {
    expect(
      isForwardableSDKToolCall({ name: "mcp", arguments: { providerIdentifier: "client" } }),
    ).toBe(false)
    expect(
      isForwardableSDKToolCall({
        name: "mcp",
    ...
    expect(isForwardableSDKToolCall({ name: "glob", arguments: { globPattern: "**/*" } })).toBe(
      true,
    )
  })
  ...
  it("requires complete non-file SDK builtin tool arguments before forwarding", () => {
    ...
    expect(
      isForwardableSDKToolCall({ name: "readLints", arguments: { paths: ["src/App.tsx"] } }),
    ).toBe(true)
    expect(
      isForwardableSDKToolCall({ name: "readLints", arguments: { filePath: "src/App.tsx" } }),
    ).toBe(true)

    expect(
      isForwardableSDKToolCall({ name: "semSearch", arguments: { targetDirectories: ["src"] } }),
    ).toBe(false)
    expect(
      isForwardableSDKToolCall({ name: "semSearch", arguments: { query: "submit button" } }),
    ).toBe(true)
    expect(
      isForwardableSDKToolCall({
        name: "semSearch",
        arguments: { search_query: "submit button", targetDirectories: ["src"] },
      }),
    ).toBe(true)

    expect(
      isForwardableSDKToolCall({ name: "todowrite", arguments: { status: "in_progress" } }),
    ).toBe(false)
    ...
    expect(
      isForwardableSDKToolCall({ name: "task", arguments: { description: "Inspect app" } }),
    ).toBe(false)
    expect(
      isForwardableSDKToolCall({
        name: "task",
        arguments: { description: "Inspect app", prompt: "Find the entrypoint" },
      }),
    ).toBe(true)

    expect(
      isForwardableSDKToolCall({ name: "createPlan", arguments: { status: "in_progress" } }),
    ).toBe(false)
    expect(
      isForwardableSDKToolCall({
        name: "createPlan",
    ...
    expect(
      isForwardableSDKToolCall({ name: "generateImage", arguments: { filePath: "out.png" } }),
    ).toBe(false)
    expect(
      isForwardableSDKToolCall({
        name: "generateImage",
    ...
    expect(
      isForwardableSDKToolCall({ name: "recordScreen", arguments: { path: "recording.mov" } }),
    ).toBe(false)
    expect(
      isForwardableSDKToolCall({ name: "recordScreen", arguments: { mode: "START_RECORDING" } }),
    ).toBe(true)
  })
  ...
  it("unwraps common dynamic client tool schema wrappers", () => {
    ...
    expect(validateClientMcpToolCall(tools, "wrapped_write_file", { path: "marker.txt" })).toBe(
      "Missing required argument for wrapped_write_file: content",
    )
    expect(
      validateClientMcpToolCall(tools, "wrapped_write_file", { path: "marker.txt", content: "ok" }),
    ).toBe(null)
  })
  ...
  it("rejects unknown or incomplete client MCP forwarding calls internally", () => {
    ...
    expect(validateClientMcpToolCall(tools, "probe_write_file", { file_path: "marker.txt" })).toBe(
      "Missing required argument for probe_write_file: contents",
    )
    expect(
  ...
  it("validates dynamic client MCP scalar constraints", () => {
    ...
    expect(validateClientMcpToolCall(tools, "constrained_search", { query: "", limit: 5 })).toBe(
      "Invalid value for constrained_search.query: expected at least 2 character(s)",
    )
    expect(
      validateClientMcpToolCall(tools, "constrained_search", { query: "todo app", limit: 5 }),
    ).toContain("expected to match pattern")
    expect(
      validateClientMcpToolCall(tools, "constrained_search", { query: "todo_app_name", limit: 5 }),
    ).toBe("Invalid value for constrained_search.query: expected at most 12 character(s)")
    expect(
      validateClientMcpToolCall(tools, "constrained_search", { query: "todo_app", limit: 0 }),
    ).toBe("Invalid value for constrained_search.limit: expected >= 1")
    expect(
      validateClientMcpToolCall(tools, "constrained_search", { query: "todo_app", limit: 55 }),
    ).toBe("Invalid value for constrained_search.limit: expected <= 50")
    expect(
      validateClientMcpToolCall(tools, "constrained_search", { query: "todo_app", limit: 7 }),
    ).toBe("Invalid value for constrained_search.limit: expected a multiple of 5")
    expect(
      validateClientMcpToolCall(tools, "constrained_search", { query: "todo_app", limit: 10 }),
    ).toBe(null)
  })
          ...
          properties: {
            ...
            input: {
              ...
              properties: {
                ...
                format: {
                  anyOf: [{ type: "string", enum: ["text", "markdown"] }, { type: "null" }],
                },
              },
  ...
  it("keeps exact custom harness MCP tools available in the subcommand server", async () => {
    ...
    expect(
      listResponse.result.tools.some((tool) => tool.name === "mcp__github__create_issue"),
    ).toBe(true)
  ...
  it("tells the SDK to forward compatible client tools through MCP", () => {
    const prompt = bridgePrompt("USER: create a file", [{ name: "bash" }, { name: "write" }])
  ...
  it("keeps SDK session affinity stable while sending client tools through MCP", () => {
    ...
    expect(baseSendOptions.mcpServers.client.env.CURSOR_SDK_BRIDGE_CLIENT_TOOLS_JSON).toContain(
      "webfetch",
    )
    expect(dynamicSendOptions.mcpServers.client.env.CURSOR_SDK_BRIDGE_CLIENT_TOOLS_JSON).toContain(
      "webfetch",
    )
    expect(dynamicSendOptions.mcpServers.client.env.CURSOR_SDK_BRIDGE_CLIENT_TOOLS_JSON).toContain(
      "probe_write_file",
