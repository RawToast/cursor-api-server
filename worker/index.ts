...
async function handleOpenAiRoute(
  ...
  if (route.kind === "models") {
    ...
    return json(
      modelList({ opencode: route.surface === "opencodev2", sdk: route.surface === "opencodev2" }),
    )
  }
...
function streamOpenAiEvents(
  ...
  const pump = async () => {
    ...
    try {
      ...
      await writer.write(
        encodeSse(
          { error: { message, type: "cursor_error", code: "cursor_stream_error" } },
          "error",
        ),
      )
    } finally {
...
function matchOpenAiRoute(pathname: string): OpenAiRoute | null {
  ...
  if (responseInputItemsMatch)
    return { kind: "responseInputItems", responseId: responseInputItemsMatch[1] }
