...
describe("OpenAI compatibility adapter", () => {
  it("converts chat messages and image URLs into Cursor prompts", () => {
        ...
        messages: [
          { role: "system", content: "Be terse." },
          {
            ...
            content: [
              ...
              {
                type: "image_url",
                image_url: { url: "https://example.com/image.png", width: 640, height: 480 },
              },
            ],
          },
        ],
    ...
    expect(prepared.prompt.images).toEqual([
      { url: "https://example.com/image.png", dimension: { width: 640, height: 480 } },
    ])
  })
  ...
  it("expands home-relative SDK paths for absolute OpenCode file schemas", () => {
    ...
    const toolCalls = toOpenAiToolCalls({
      ...
      toolCalls: [
        {
          name: "write",
          arguments: { path: "~/Desktop/rain-in-spain.html", fileText: "<main>Rain</main>" },
        },
      ],
    })
  ...
  it("maps the live OpenCode build tool matrix including unique client tools", () => {
      ...
      },
      {
        ...
        parameters: {
          ...
          properties: {
            url: { type: "string" },
            format: {
              anyOf: [{ type: "string", enum: ["text", "markdown", "html"] }, { type: "null" }],
            },
            timeout: { type: "number" },
          },
          required: ["url"],
    ...
    const toolCalls = toOpenAiToolCalls({
      ...
      toolCalls: [
        {
          name: "shell",
          arguments: { command: "npm test", timeout: 120_000, workingDirectory: "/tmp/project" },
        },
        { name: "read", arguments: { path: "src/App.tsx", offset: 5, limit: 20 } },
        {
          name: "write",
        ...
        {
          name: "todowrite",
          arguments: { todos: [{ content: "Build app", status: "in_progress", priority: "high" }] },
        },
        ...
        { name: "semsearch", arguments: { query: "submit button", targetDirectories: ["src"] } },
        {
          ...
          arguments: {
            providerIdentifier: "client",
            toolName: "webfetch",
            args: { url: "https://example.com", format: "markdown", timeout: 10 },
          },
        },
        {
          name: "mcp",
    ...
    expect(toolCalls.map((call) => JSON.parse(call.function.arguments))).toEqual([
      ...
      { pattern: "*", path: "src" },
      {
        command: "rg --line-number --color never --hidden 'submit button' 'src'",
  ...
  it("maps a Vite React app-build flow through strict OpenCode schemas", () => {
    ...
    const generated = toOpenAiToolCalls({
      ...
      toolCalls: [
        ...
        {
          name: "edit",
          arguments: { path: "package.json", oldString: '"scripts": {', newString: '"scripts": {' },
        },
            ...
            tool_calls: [
              {
                id: "call_1",
                type: "function",
                function: { name: "write", arguments: '{"filePath":"index.html"}' },
              },
            ],
          },
          {
            ...
            tool_calls: [
              {
                id: "call_ls",
                type: "function",
                function: { name: "bash", arguments: '{"command":"pwd && ls -la"}' },
              },
            ],
          },
          {
    ...
    const prepared = prepareOpencodeSdkChatRequest(
      {
        ...
        messages: [
          {
            role: "user",
            content: "Please save rain-in-spain.html on ~/Desktop with a short paragraph.",
          },
        ],
        tools: [
          {
    ...
    const prepared = prepareResponsesRequest(
      {
        ...
        input: [
          {
            role: "user",
            content: [{ type: "input_text", text: "build a todo app in vite 8 and react" }],
          },
          {
            type: "function_call",
            call_id: "call_patch",
  ...
  it("normalizes Cursor-style tool names and arguments to the client schema", () => {
    const toolCalls = toOpenAiToolCalls({
      ...
      toolCalls: [
        ...
        {
          name: "edit_file",
          arguments: { path: "index.html", old_string: "Hello", new_contents: "Hi" },
        },
  ...
  it("maps SDK calls through composed and wrapped JSON schemas", () => {
    ...
    const toolCalls = toOpenAiToolCalls({
      ...
      toolCalls: [
        {
          name: "shell",
          arguments: { command: "npm run build", workingDirectory: "/workspace", timeout: 120_000 },
        },
        {
          name: "write",
          arguments: {
          ...
          { role: "user", content: "build a todo app" },
          {
            ...
            tool_calls: [
              ...
              },
              {
                ...
                function: {
                  name: "glob",
                  arguments: JSON.stringify({ pattern: "**/*.tsx", path: "/tmp/project/src" }),
                },
              },
              {
                id: "call_todo",
  ...
  it("drops synthetic SDK shell workdirs so OpenCode uses its local cwd", () => {
    const toolCalls = toOpenAiToolCalls({
      ...
      toolCalls: [
        {
          name: "shell",
          arguments: { command: "npm install && npm test", workingDirectory: "/workspace" },
        },
      ],
    })
  ...
  it("fills required shell cwd and timeout fields for strict harness command tools", () => {
    const toolCalls = toOpenAiToolCalls({
      ...
      toolCalls: [
        { name: "shell", arguments: { command: "npm test", workingDirectory: "/workspace" } },
      ],
    })
  ...
  it("preserves recursive Cursor targeting without leaking absolute target paths into glob patterns", () => {
    const toolCalls = toOpenAiToolCalls({
      ...
      toolCalls: [
        {
          name: "file_search",
          arguments: { targeting: "/Users/example/project/**", glob_pattern: "*.ts" },
        },
      ],
    })
  ...
  it("repairs swapped SDK glob pattern and path values", () => {
    const toolCalls = toOpenAiToolCalls({
      ...
      toolCalls: [
        { name: "glob", arguments: { targetDirectory: "**/*.tsx", globPattern: "/tmp/project" } },
      ],
    })
  ...
  it("maps SDK file operations to apply-patch style tools", () => {
    ...
    expect(toolCalls.map((call) => JSON.parse(call.function.arguments))).toEqual([
      ...
      {
        path: "src/old.tsx",
        patch: ["*** Begin Patch", "*** Delete File: src/old.tsx", "*** End Patch"].join("\n"),
      },
    ])
  })
