...
describe("Worker", () => {
  it("allows OpenCode session headers in CORS preflight", async () => {
    ...
    const response = await handleRequest(
      new Request("https://composer.test/opencodev2/v1/chat/completions", { method: "OPTIONS" }),
      env,
      fakeCtx(),
      deps,
    )
      ...
      choices: [
        {
          message: {
            ...
            tool_calls: [
              { type: "function", function: { name: "glob", arguments: '{"glob_pattern":"*"}' } },
            ],
          },
  ...
  it("can route OpenCode SDK runs through a standard streaming bridge", async () => {
    ...
    expect(String((sdkRequests[0].body as { prompt?: string }).prompt || "")).toContain(
  ...
  it("labels the OpenCode model without changing the standard model list", async () => {
    ...
    const standardBody = (await standard.json()) as {
      data: Array<{ id: string; name: string; cost?: { input: number; output: number } }>
    }
    const opencodeSdkBody = (await opencodeSdk.json()) as {
      data: Array<{ id: string; name: string; cost?: { input: number; output: number } }>
    }
    expect(standardBody.data.find((model) => model.id === "composer-2.5")?.name).toBe(
      "Cursor Composer 2.5",
    )
    expect(standardBody.data.map((model) => model.id)).not.toContain("composer-2.5-sdk")
    expect(opencodeSdkBody.data.find((model) => model.id === "composer-2.5")?.name).toBe(
      "Composer 2.5",
    )
    expect(opencodeSdkBody.data.find((model) => model.id === "composer-2.5-sdk")?.name).toBe(
      "Composer 2.5 SDK Harness",
    )
    expect(opencodeSdkBody.data.find((model) => model.id === "composer-2.5")?.cost).toEqual({
      input: 0.5,
      output: 2.5,
    })
  })
  ...
  it("returns a buffered JSON response for /v1/responses when stream is absent", async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({
      object: "response",
      output: [
        { type: "message", content: [{ type: "output_text", text: "Hello from Composer" }] },
      ],
    })
  })
        ...
        return new Response(
          new ReadableStream<Uint8Array>({
            start(controller) {
              controller.enqueue(
                connectFrame(
                  chatResponseThinking("The answer is simple.</think>\nHello from Composer"),
                ),
              )
              controller.enqueue(connectFrame(new TextEncoder().encode("{}"), 2))
              controller.close()
...
function sdkRunKind(
  body: string,
): "completed" | "drop" | "hello" | "invalid" | "list" | "shell" | "tool-result" {
  const text = body
  ...
  return new Response(
    new ReadableStream<Uint8Array>({
      start(controller) {
        if (kind === "list") {
          controller.enqueue(
            localSdkToolCallFrame("sdk_call_1", 4, protoMessage([protoField(2, "*")])),
          )
          ...
          controller.enqueue(
            localSdkToolCallFrame(
              ...
              protoMessage([
                ...
                protoField(
                  2,
                  protoValueMapEntry("body", protoStringValue("Missing required title")),
                ),
                protoField(4, "github"),
          ...
          controller.enqueue(
            localSdkExecFrame(
              1,
              2,
              protoMessage([protoField(1, "npm test"), protoField(2, "/workspace")]),
            ),
          )
