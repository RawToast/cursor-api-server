...
describe("Cursor stream adapter", () => {
  ...
  it("strips Composer final markers when there is no think closing tag", async () => {
    const response = new Response(
      new ReadableStream<Uint8Array>({
        start(controller) {
          controller.enqueue(
            connectFrame(chatResponseThinking("Hidden reasoning <｜final｜>Visible answer")),
          )
          controller.enqueue(connectFrame(new TextEncoder().encode("{}"), 2))
          controller.close()
        },
  ...
  it("strips Composer final markers from normal text frames", async () => {
    const response = new Response(
      new ReadableStream<Uint8Array>({
        start(controller) {
          controller.enqueue(
            connectFrame(chatResponseText("Hidden reasoning <｜final｜>\nVisible answer")),
          )
    ...
    expect(events).toEqual([
      ...
      },
      {
        ...
        toolCalls: [
          {
            name: "Glob",
            arguments: { targeting: "/Users/example/project/**", glob_pattern: "*" },
          },
        ],
      },
    ])
  ...
  it("parses inline Composer tool-call arguments", () => {
    expect(
      ...
      { name: "Glob", arguments: { targeting: "/Users/example/project/**", glob_pattern: "*" } },
    ])
  })
  ...
  it("parses alternate JSON tool-call body shapes", () => {
    expect(
      ...
      { name: "write_file", arguments: { target_file: "index.html", new_contents: "hi" } },
    ])
    ...
    const response = new Response(
      new ReadableStream<Uint8Array>({
        start(controller) {
          ...
          controller.enqueue(
            encodeSse({ type: "text-delta", text: " world" }, "interaction_update"),
          )
