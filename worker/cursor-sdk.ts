...
async function cursorLocalSdkRaw(
  ...
  if (!base)
    throw new HttpError("Cursor backend URL is not configured", 500, "cursor_missing_backend_url")
  ...
  if (!response.ok) {
    ...
    const message =
      response.status === 401
        ...
        : parsed.message || `Cursor local SDK request failed with status ${response.status}`
    const status =
      response.status === 401
        ? 401
        : response.status === 429
    ...
    throw new HttpError(
      message,
      status,
      response.status === 401 ? "cursor_unauthorized" : parsed.code || "cursor_sdk_error",
    )
  }
  return response
}
...
async function cursorLocalSdkBridgeJson(
  ...
  if (!response)
    throw new HttpError("Cursor SDK bridge is not configured", 500, "cursor_sdk_bridge_missing")
...
async function parseCursorLocalSdkBridgeJsonResponse(
  ...
  if (!response.ok) {
    ...
    const code =
      typeof error?.code === "string" && error.code ? error.code : "cursor_sdk_bridge_error"
    const status =
      response.status === 401
        ? 502
        : response.status === 429
...
function withSdkStartTimeout<T>(promise: Promise<T>): Promise<T> {
  return new Promise<T>((resolve, reject) => {
    const timer = setTimeout(() => {
      reject(
        new HttpError("Cursor local SDK stream did not start.", 504, "cursor_sdk_stream_timeout"),
      )
...
function decodeLocalAgentServerFrame(payload: Uint8Array): LocalSdkDecodedEvent[] {
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Could not decode Cursor local SDK stream"
...
function decodeExecServerToolCall(
  ...
  for (const field of fields) {
    ...
    const toolCallId =
      stringArg(args, "toolCallId") || execId || `exec_${id ?? stableToolCallId(payload)}`
