...
export function prepareChatRequest(
  ...
  const model =
    typeof record.model === "string" && record.model.trim() ? record.model.trim() : "composer-2.5"
  ...
  const workspaceMutationDone =
    workspaceMutationRequired && hasRequiredLocalToolCall(messages, tools, latestUserText)
  const transcript: string[] = [
    tools.length ? TOOL_SYSTEM_DIRECTIVE : agentMode ? AGENT_SYSTEM_DIRECTIVE : SYSTEM_DIRECTIVE,
  ]
...
export function responseTextStartEvents(input: { id: string; outputIndex: number }): Uint8Array[] {
  ...
  return [
    encodeSse(
      { type: "response.output_item.added", output_index: input.outputIndex, item },
...
export function responseToolCallEvents(input: {
  ...
  return [
    ...
    encodeSse(
      { type: "response.output_item.done", output_index: input.outputIndex, item: doneItem },
  ...
  const textEvents =
    input.textStarted || !(input.toolCalls ?? []).length
      ? [
          ...
          encodeSse(
            { type: "response.output_item.done", output_index: input.textOutputIndex ?? 0, item },
            "response.output_item.done",
          ),
        ]
      : []
...
function parseChatTools(value: unknown): OpenAiToolSpec[] {
  ...
  if (!Array.isArray(value))
    throw new HttpError("tools must be an array.", 400, "invalid_request_error", "tools")
...
function sdkRoutingSamples(): CursorToolCall[] {
  return [
    {
      name: "shell",
      arguments: { command: "<command>", workingDirectory: "/workspace", timeout: 120_000 },
    },
    { name: "read", arguments: { path: "src/App.tsx", offset: 1, limit: 80 } },
    { name: "write", arguments: { path: "src/App.tsx", fileText: "<file content>" } },
    {
      name: "edit",
      arguments: { path: "src/App.tsx", oldString: "<old text>", newString: "<new text>" },
    },
    ...
    {
      name: "todowrite",
      arguments: { todos: [{ content: "<task>", status: "in_progress", priority: "medium" }] },
    },
  ]
}
...
function appendChatOptions(transcript: string[], record: Record<string, unknown>) {
  ...
  if (constraints.length)
    transcript.push("", "OUTPUT CONSTRAINTS:", ...constraints.map((item) => `- ${item}`))
}
...
function appendJsonConstraint(constraints: string[], format: unknown) {
  ...
  if (format.type === "json_object")
    constraints.push("Return a single valid JSON object and no surrounding prose.")
...
function responseInputToTextAndImages(
  ...
  if (!Array.isArray(input))
    return { text: input === undefined ? "" : JSON.stringify(input), images: [] }
  const lines: string[] = []
  const images: CursorImage[] = []
  const toolCallById = new Map<string, { name: string; args: Record<string, unknown> }>()
...
function responseInputWithPrevious(
  ...
  const previous = [...(options.previousInputItems ?? []), ...(options.previousOutput ?? [])]
...
function shouldRequireLocalTool(text: string, tools: OpenAiToolSpec[]): boolean {
  ...
  const wantsFileMutation =
    /\b(create|write|save|overwrite|edit|modify|update|delete|remove|make)\b/.test(lower)
...
function looksLikeWorkspaceMutationArguments(args: Record<string, unknown>): boolean {
  for (const record of argumentRecords(args)) {
    ...
    if (oldText !== undefined && newText !== undefined && (!operation || mutatingOperation))
      return true
...
function inferSdkCanonicalFromClientTool(
  ...
  if (
    ...
    firstStringArg(args, "pattern", "query", "search", "regex", "searchPattern", "search_pattern")
  )
    return "grep"
  if (
    tool &&
...
function openCodeArgsToSdkArgs(
  ...
  const mcpTarget =
    canonical === "mcp" ? mcpTargetForClientToolName(toolName, { includeMapped: true }) : undefined
  if (mcpTarget) {
    return {
...
function sdkTimeoutArgument(
  ...
  const target =
    firstMatchingProperty(timeoutCandidates(), schema.properties, normalizedProperties) ||
    argument.key
...
function serializedToolCallLength(toolCalls: OpenAiToolCall[]): number {
  return toolCalls.reduce(
    (sum, toolCall) => sum + toolCall.function.name.length + toolCall.function.arguments.length,
    0,
  )
}
...
function resolveToolSpec(
  ...
  const alias = tools.find(
    (tool) =>
      candidates.includes(normalizeToolName(tool.name)) && schemaLooksCompatible(emittedName, tool),
  )
...
function canonicalToolName(value: string): string {
  ...
  if (["search", "searchfiles", "searchfilesystem", "ripgrep", "rg"].includes(normalized))
    return "grep"
  ...
  if (
    ["updatetodos", "updatetodostoolcall", "writetodos", "todowrite", "todowritetoolcall"].includes(
      normalized,
    )
  )
    return "todowrite"
...
function schemaLooksCompatible(emittedName: string, tool: OpenAiToolSpec): boolean {
  ...
  const has = (candidates: string[]) =>
    Boolean(firstMatchingProperty(candidates, schema.properties, normalizedProperties))
  ...
  switch (canonical) {
    ...
    case "edit":
      return has(pathCandidates()) && has(oldTextCandidates()) && has(newTextCandidates())
    case "grep":
...
function schemaCompatibilityScore(emittedName: string, tool: OpenAiToolSpec): number {
  ...
  if (toolNameAliases(normalizeToolName(emittedName)).includes(normalizeToolName(tool.name)))
    return 95
...
function canEmulateWithShell(emittedName: string): boolean {
  return ["write", "read", "edit", "delete", "grep", "glob", "ls", "semsearch"].includes(
    canonicalToolName(emittedName),
  )
}
...
function commandStyleFileArguments(
  ...
  if (emittedCanonical === "write") {
    ...
    copyOptionalArgument(output, schema.properties, normalizedProperties, args, [
      "offset",
      "start",
      "startLine",
      "start_line",
...
function toolArgumentsSatisfySchema(args: Record<string, unknown>, tool: OpenAiToolSpec): boolean {
  ...
  for (const required of schema.required) {
    const property =
      firstMatchingProperty([required], schema.properties, normalizedProperties) ?? required
    if (!argumentValueSatisfiesSchema(args[property], schema.propertySchemas[property], true))
      return false
  }
  for (const [property, value] of Object.entries(args)) {
    ...
    if (!argumentValueSatisfiesSchema(value, schema.propertySchemas[propertyName], false))
      return false
  }
  return true
}
...
function argumentValueSatisfiesSchema(value: unknown, schema: unknown, required: boolean): boolean {
  ...
  if (anyOf.length && !anyOf.some((item) => argumentValueSatisfiesSchema(value, item, true)))
    return false
  const oneOf = composedToolSchemas(record.oneOf)
  if (oneOf.length && !oneOf.some((item) => argumentValueSatisfiesSchema(value, item, true)))
    return false
  const allOf = composedToolSchemas(record.allOf)
  if (allOf.length && !allOf.every((item) => argumentValueSatisfiesSchema(value, item, true)))
    return false
  const types = schemaJsonTypes(record)
  if (types.length && !types.some((type) => jsonValueMatchesType(value, type))) return false
  if (objectConstraintsApply(record, value, types) && !objectValueSatisfiesSchema(value, record))
    return false
  if (arrayConstraintsApply(record, value, types) && !arrayValueSatisfiesSchema(value, record))
    return false
  return true
}
...
function objectValueSatisfiesSchema(value: unknown, schema: Record<string, unknown>): boolean {
  ...
  for (const requiredProperty of required) {
    const property =
      firstMatchingProperty([requiredProperty], propertyNames, normalizedProperties) ??
      requiredProperty
    if (!argumentValueSatisfiesSchema(recordValue[property], properties[property], true))
      return false
  }
  for (const [property, nestedValue] of Object.entries(recordValue)) {
    ...
    if (
      ...
      !argumentValueSatisfiesSchema(nestedValue, schema.additionalProperties, false)
    )
      return false
  }
  return true
  ...
  return schema.required
    .flatMap((property) => {
      const canonicalProperty =
        schema.properties.find((item) => normalizeToolName(item) === normalizeToolName(property)) ??
        property
...
function requiredArgumentSummaryForSchema(prefix: string, schema: unknown): string[] {
  ...
  if (nestedNames.length && nestedRequired.length) {
    ...
    return nestedRequired.flatMap((property) => {
      const canonicalProperty =
        firstMatchingProperty([property], nestedNames, normalizedProperties) ?? property
      return requiredArgumentSummaryForSchema(
...
function toolSchemaPropertySummary(tool: OpenAiToolSpec): string {
  ...
  return schema.properties
    .map((property) => `${property}:${schemaTypeLabel(schema.propertySchemas[property])}`)
    .join(", ")
}
...
function normalizeMCPWrapperArguments(
  ...
  const serverKey = firstMatchingProperty(
    ["serverName", "server", "provider", "providerIdentifier", "provider_identifier"],
    schema.properties,
    normalizedProperties,
  )
...
function normalizeWrapperObjectArguments(
  ...
  const nested = normalizeToolArguments(
    ...
    canonicalToolName(emittedName) === "mcp" && canonicalToolName(tool.name) !== "mcp"
      ? tool.name
      : emittedName,
    wrapperDepth + 1,
    context,
...
function canonicalToolSchemaRecord(
  ...
  if (!isRecord(dereferenced.properties)) {
    if (isRecord(dereferenced.schema))
      return canonicalToolSchemaRecord(dereferenced.schema, root, depth + 1, seenRefs)
    if (isRecord(dereferenced.json_schema))
      return canonicalToolSchemaRecord(dereferenced.json_schema, root, depth + 1, seenRefs)
  }
  return dereferenced
}
...
function directToolParameterSchema(
  ...
  return {
    ...
    allowAdditionalProperties:
      parameters.additionalProperties === true || isRecord(parameters.additionalProperties),
    propertySchemas,
  }
}
...
function mergeToolParameterSchemas(
  ...
  const useful = shapes.filter(
    (shape) => shape.properties.length || shape.required.length || shape.allowAdditionalProperties,
  )
...
function applyRequiredToolDefaults(
  ...
  if (isShellLikeTool(tool, originalArgs)) {
    ...
    if (required.includes("description") && typeof next.description !== "string") {
      const command =
        commandLikeValue(next, tool) ?? firstStringArg(originalArgs, ...shellCommandCandidates())
...
function sanitizeNormalizedToolArguments(
  ...
  for (const candidate of shellExplicitWorkdirCandidates()) {
    const key =
      firstMatchingProperty([candidate], schema.properties, normalizedProperties) ?? candidate
...
function isShellLikeTool(
  ...
  if (
    ...
    return true
  return Boolean(
    commandLikeProperty(tool) && firstStringArg(originalArgs, ...shellCommandCandidates()),
  )
}
...
function aliasToolArgument(
  ...
  const normalized = normalizeToolName(key)
  const rules = [
    ...toolSpecificArgumentAliases(normalizeToolName(toolName || ""), normalized),
...
function commonArgumentAliases(
  ...
  const aliases: Record<string, Array<{ candidates: string[]; priority: number }>> = {
    ...
    directory: [
      { candidates: [...shellWorkdirCandidates(), ...globPathCandidates()], priority: 45 },
    ],
    ...
    newcontents: [
      { candidates: [...newTextCandidates(), ...fileContentCandidates()], priority: 85 },
    ],
  ...
  if (normalized === "path")
    return [{ candidates: ["filePath", "path", "directory", "cwd", "pattern"], priority: 75 }]
  if (normalized === "prompt")
    return [{ candidates: ["prompt", "description", "instructions", "query"], priority: 80 }]
...
function toolSpecificArgumentAliases(
  ...
  if (["grep", "search", "searchfiles"].includes(tool)) {
    ...
    if (["caseinsensitive", "ignorecase"].includes(normalized)) {
      return [
        {
          candidates: ["ignoreCase", "ignore_case", "caseInsensitive", "case_insensitive"],
          priority: 95,
        },
      ]
    }
    ...
    if (["headlimit", "limit", "maxresults", "maxresult"].includes(normalized)) {
      return [
        {
          candidates: ["limit", "headLimit", "head_limit", "maxResults", "max_results"],
          priority: 90,
        },
      ]
    }
    ...
    if (["contextbefore", "beforecontext"].includes(normalized)) {
      return [
        {
          candidates: ["contextBefore", "context_before", "beforeContext", "before_context"],
          priority: 90,
        },
      ]
    }
    if (["contextafter", "aftercontext"].includes(normalized)) {
      return [
        {
          candidates: ["contextAfter", "context_after", "afterContext", "after_context"],
          priority: 90,
        },
      ]
    }
  ...
  if (["write", "writefile", "createfile"].includes(tool)) {
    ...
    if (
      ["newcontents", "contents", "content", "text", "body", "data", "value"].includes(normalized)
    ) {
  ...
  if (["bash", "shell", "terminal", "runterminalcmd"].includes(tool)) {
    ...
    if (
      ["timeout", "timeoutms", "timeoutmilliseconds", "timeoutseconds", "seconds"].includes(
        normalized,
      )
    ) {
...
function isGlobLikeToolName(normalized: string): boolean {
  return ["glob", "fileglob", "filesearch", "find", "findfile", "findfiles", "globfiles"].includes(
    normalized,
  )
}
...
function expectRecord(value: unknown, name: string): Record<string, unknown> {
  if (!isRecord(value))
    throw new HttpError(`${name} must be an object`, 400, "invalid_request_error", name)
  return value
}

function expectArray(value: unknown, name: string): unknown[] {
  if (!Array.isArray(value))
    throw new HttpError(`${name} must be an array`, 400, "invalid_request_error", name)
  return value
}
