...
function cursorChatEndpoint(env: Env): string {
  ...
  if (!endpoint)
    throw new HttpError("Cursor chat endpoint is not configured", 500, "cursor_missing_endpoint")
  return endpoint
}
...
function encodeCursorChatRequest(input: {
  ...
  const cursorSetting = protoMessage([
    ...
    protoField(
      6,
      2,
      protoMessage([protoField(1, 2, new Uint8Array(0)), protoField(2, 2, new Uint8Array(0))]),
    ),
    protoField(8, 0, 1),
    protoField(9, 0, 1),
  ])
...
async function resolveCursorImages(
  ...
  for (const [index, image] of images.entries()) {
    ...
    if (!data.length)
      throw new HttpError("Image input is empty.", 400, "invalid_request_error", "image")
  ...
  try {
    for (const field of decodeProtobufFields(payload)) {
      if (field.no === 1) {
        return {
          type: "tool_call",
          ...(field.value instanceof Uint8Array ? decodeBinaryToolCall(field.value) : {}),
        }
      }
    ...
    return {
      type: "error",
      message: error instanceof Error ? error.message : "Failed to decode Cursor stream",
    }
  }
}
...
const TOOL_MARKER_CANDIDATES = [
  ...
  TOOL_CALL_END,
  TOOL_SEP,
].flatMap((marker) => [marker, marker.replaceAll("|", "｜").replaceAll("_", "▁")])
